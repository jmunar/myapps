...
| Investment data  | Indexa Capital REST API (read-only) |
...
- `/leanfin/` — LeanFin sub-app (nested router)
  ...
  - `/leanfin/accounts` — Account management (bank + Indexa + manual)
...
Audit log of every notification generated by the system. Persisted before
delivery is attempted so alerts are recorded even if VAPID is unconfigured or
delivery fails.
...
LeanFin tracks investment accounts held at Indexa Capital (depositary:
Cecabank). PSD2 covers payment accounts, not securities/custody accounts, so
Enable Banking cannot see these — before this integration they could only be
tracked as manual accounts with hand-entered values.

### Per-User Token

Indexa uses a long-lived **personal API token**, generated by the user in
Indexa's private area (Ajustes → Aplicaciones) and sent as the `X-AUTH-TOKEN`
header. It is read-only (no trading) and covers every account the user holds or
co-holds. The token is stored per user in `leanfin_user_settings.indexa_token`,
encrypted at rest with AES-256-GCM using the same `ENCRYPTION_KEY` scheme as the
Enable Banking private key.

Unlike a PSD2 consent, the token does not expire. Indexa accounts therefore use
a far-future `session_expires_at` sentinel (`9999-12-31`) and never surface
...
| Endpoint | Purpose |
|----------|---------|
| `GET /users/me` | List linkable accounts (link flow only) |
| `GET /accounts/{n}/portfolio` | Current valuation — the only call the routine sync makes |
| `GET /accounts/{n}/performance` | Daily valuation history, for the one-off backfill at link time |

### Personal Data

`GET /users/me` and `GET /accounts/{n}` embed the user's postal address, email
and national ID document numbers, despite Indexa's documentation stating the API
excludes personal data. Consequently:

- Neither response is written to `leanfin_api_payloads`.
- The routine sync never calls either endpoint.
- Response structs deserialize only the fields LeanFin needs; serde discards the
  rest.

`/portfolio` and `/performance` carry no personal data and are payload-logged as
usual, under `provider = 'indexa'`.

### Account Linking Flow

1. User saves an Indexa API token at `/leanfin/settings`.
2. User opens `/leanfin/accounts/indexa/link`. (Without a token they are
   redirected to settings.)
3. The app calls `GET /users/me` and renders the linkable accounts as
   `acct_{number}` checkboxes. Account numbers already linked are shown as such.
4. `POST /leanfin/accounts/indexa/link` re-fetches the account list and accepts
   only numbers the token actually owns, then inserts rows with
   `account_type = 'indexa'` and `account_uid = <account_number>`.
5. The full valuation history is backfilled from `performance.portfolios[]`,
   which reaches back to account opening. PSD2 offers 90 days at best.

### Sync Job Flow (cron)

Indexa accounts sync independently of Enable Banking — a user with only Indexa
accounts needs no Enable Banking configuration at all.

```
myapps cron
  │
  ├─ (Enable Banking bank accounts — see above)
  │
  └─ For each user with account_type = 'indexa', archived = 0:
      ├─ Decrypt the user's Indexa token (skip user if absent)
      │
      └─ For each Indexa account:
          ├─ GET /accounts/{n}/portfolio
          ├─ UPDATE leanfin_accounts.balance_amount (currency EUR)
          └─ Record balance snapshot (balance_type = 'INDEXA', end-of-day
             timestamp from the portfolio's own valuation date)
```

There is no reconciliation step: Indexa reports a valuation, not a running
balance with transactions to reconcile against.

### Balance Series

Indexa accounts have balance snapshots but no transactions, so
`get_balance_series` routes every non-bank account type through the
snapshot-with-gap-filling path. The bank path reconstructs balances by walking
transactions backwards from a snapshot and would yield nothing here.
